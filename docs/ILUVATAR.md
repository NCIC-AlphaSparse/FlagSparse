...
| 自动探测 | 三个信号，任一命中即可（`_detect_iluvatar_runtime()`）：torch 版本号带 `+corex` 标记、`COREX_HOME` / `COREX_PATH` 环境变量、设备名含 `iluvatar` / `bi-v` / `corex` |
...
- **`+corex` 版本标记不可靠**：那台机器厂商镜像里的 torch 报的是**光秃秃的 `2.10.0`**，没有后缀。
  保留这个信号是因为别的 CoreX wheel 确实带，但它一个人顶不住；
- **设备名要运行时可用才读得到**：同一台机器 CUDA 初始化失败（Error 803），
  `get_device_properties()` 直接抛异常，这个信号也没了；
- **`COREX_HOME` 是两种情况下都还在的那个**，也是 C API 的 `IX` 槽位读的同一个变量
...
| 容器里的 torch | `2.10.0`，**没有 `+corex` 后缀**（探测的信号 1 因此失效，已改用 `COREX_HOME` 兜底） |
...
| 自动探测能否命中（现在只剩 `COREX_HOME` 这一条路） | 未验证 |
